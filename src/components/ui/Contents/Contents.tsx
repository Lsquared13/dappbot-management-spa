...
export function getMainDir(
  flyoutSize: Flyout,
  idealDirection: MainDir | undefined,
  triggerRect: ClientRect,
  windowSize: Window
) {
...
export function adjustOffsets(
  ...
  flyoutOffset: ContentsState["flyoutOffset"];
  caretOffset: ContentsState["caretOffset"];
} {
  let flyoutLeft = base.left;
  let flyoutTop =  triggerRect.height - flyoutSize.height / 2;

  let caretTop = mainDir === "down" ? -CARET_HEIGHT : undefined;
  let caretRight = mainDir === "left" ? -CARET_HEIGHT : undefined;
  let caretBottom = undefined;
  let caretLeft = mainDir === "right" ? -CARET_HEIGHT : undefined;
...
> {
  ...
  componentDidMount() {
    ...
    setTimeout(() => {
      if (this.props.shouldFocus && this.flyout && this.flyout.current) {
  ...
  componentWillReceiveProps(nextProps: ContentsProps) {
   ...
   */
  setFlyoutPosition = (props: ContentsProps) => {
    const {
      relativeOffset,
      idealDirection,
    ...
    const flyoutSize = {
      height:
        this.flyout && this.flyout.current
          ? this.flyout.current.clientHeight
          : 0,
      width
    };
  ...
  flyout = React.createRef<HTMLElement>();
    ...
    return (
      ...
      >
        <div
          ...
          ref={this.flyout as any}
          tabIndex={-1}
        >
          <div
            className={classnames("dimensions", "innerContents")}
            style={{ width }}
          >
            {children}
          </div>
          <div
            className={classnames(bgColor, "caret")}
